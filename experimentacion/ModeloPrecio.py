...
class ModeloPrecio:
    ...
    def fit(self):
        for name, group in self.grouped:
            ...
            self.linear_regressor_segmentos[name] = metnum.LinearRegression()
            self.linear_regressor_segmentos[name].fit(sinPrecio, precios)

    def predict(self, dfToPred):
        ...
        for name, group in segmentedPred:
            ...
            group["precio"] = self.linear_regressor_segmentos[name].predict(group.values)
            result.append(group)
