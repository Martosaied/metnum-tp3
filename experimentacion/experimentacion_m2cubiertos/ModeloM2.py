...
class ModeloM2(ModeloM2Abstract):
    ...
    def fit(self):
        ...
        y = self.df['metroscubiertos'].values
        y = y.reshape(len(y), 1)
    ...
    def predict(self):
        ...
        response['metroscubiertos'] = self.linear_regressor_instance.predict(
            response)
        return response
...
class Modelometroscubiertoseg(ModeloM2Abstract):
    ...
    def predict(self):
        ...
        for name, group in self.df_predict_grouped:
            ...
            mp = ModeloM2(fit_df, self.picked_columns)
            df_predicted = mp.run(group)

            result.append(df_predicted)
        self.df_predict = pd.concat(result).sort_index() 
        return self.df_predict['metroscubiertos'].fillna(2).apply(round)
...
class ModeloV2(ModeloM2Abstract):
    ...
    def clean(self, df: DataFrame, dropna=True):
        ...
        df = df[self.columnas_piolas + ['metroscubiertos']]
        if dropna:
    ...
    def fit_model(self, gName, group):
        ...
        self.linear_regressor_segmentos[gName].fit(metroscubiertos , metroscubiertoss)
    ...
    def predict(self, dfToPred: DataFrame):
        ...
        for gName, group in segmentedPred:
            ...
            metroscubiertos = np.c_[group[self.columnas_piolas].values, np.ones(group.values.shape[0])] if self.termino_independiente else group[self.columnas_piolas].values
            group["metroscubiertos"] = self.linear_regressor_segmentos[gName].predict(metroscubiertos)
            result.append(group)
